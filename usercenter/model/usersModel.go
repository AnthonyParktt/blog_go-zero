...
type (
	...
	UsersModel interface {
		...
		FindOneByUname(ctx context.Context, uname string) (*Users, error)
	}
...
func (m *customUsersModel) FindOneByUname(ctx context.Context, uname string) (*Users, error) {
	query := fmt.Sprintf("select %s from %s where `username` = ? limit 1", usersRows, m.table)
	var resp Users
	err := m.conn.QueryRowCtx(ctx, &resp, query, uname)
	switch err {
	case nil:
		return &resp, nil
